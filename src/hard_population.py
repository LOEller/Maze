from hard_maze_runner import HardMR
import math
import copy
import random
import copy
...
class HardPopulation:
    def __init__ (self, win, maze, startX, startY, direction, endX, endY):
        ...
        for i in range(100):
            self.population[i] = HardMR(win, startX, startY, direction, maze)
    ...
    def crossover (self, parent1, parent2):
        crossPoint = random.randint(1, 63)
        result = HardMR(self.win, self.startX, self.startY, self.dir, self.maze)
        for i in range(1, 63):
            if i <= crossPoint:
    ...
    def stats (self):
        ...
        print(f"Total: {np.sum(fitnesses)} , Median: {np.median(fitnesses)} Mean: {np.mean(fitnesses)}")

        self.averageFitness = np.mean(fitnesses)
